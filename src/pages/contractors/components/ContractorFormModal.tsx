...
import { validatePositiveNumber, validateNumber } from '@/shared/utils/number-validation';
import { useUIStore } from '@/store/ui';
import { 
	CONTRACTOR_FORM_MODAL_MAX_WIDTH,
...
/**
 * Компонент модального окна формы подрядчика
 */
export function ContractorFormModal({
	...
	onCancel,
	onClose,
}: ContractorFormModalProps): React.ReactElement {
	...
	const handleRateBlur = () => {
		if (rate.trim()) {
			const validation = validatePositiveNumber(rate, {
				required: false,
				fieldName: 'Ставка',
			});
			if (!validation.valid) {
				setRateError(validation.error || null);
			} else {
				setRateError(null);
			}
		} else {
	...
	const handleRatingBlur = () => {
		if (rating.trim()) {
			const validation = validateNumber(rating, {
				min: 1,
				max: 5,
				allowZero: false,
				allowNegative: false,
			...
			if (!validation.valid) {
				setRatingError(validation.error || null);
			} else {
				setRatingError(null);
			}
		} else {
	...
	const handleFormSubmit = async (e: React.FormEvent) => {
		...
		// Валидация ставки
		if (rate.trim()) {
			const rateValidation = validatePositiveNumber(rate, {
				required: false,
				fieldName: 'Ставка',
			});
			if (!rateValidation.valid) {
				setRateError(rateValidation.error || null);
				await useUIStore.getState().showAlert({
					title: UI_TEXTS.VALIDATION_ERROR,
					message: rateValidation.error || 'Некорректная ставка',
				});
				return;
			}
		}
		...
		// Валидация рейтинга
		if (rating.trim()) {
			const ratingValidation = validateNumber(rating, {
				min: 1,
				max: 5,
				allowZero: false,
				allowNegative: false,
			...
			if (!ratingValidation.valid) {
				setRatingError(ratingValidation.error || null);
				await useUIStore.getState().showAlert({
					title: UI_TEXTS.VALIDATION_ERROR,
					message: ratingValidation.error || 'Некорректный рейтинг',
				});
				return;
			}
		}
	...
	return (
		<div className="modal-backdrop" onClick={onClose}>
			...
			>
				<h3 style={{ marginTop: 0 }}>{editing ? UI_TEXTS.EDIT_CONTRACTOR : UI_TEXTS.NEW_CONTRACTOR}</h3>
				<form onSubmit={handleFormSubmit} className="form-grid">
					...
					<label className="col-span">
						...
						<TextInput 
							...
							onChange={(e) => onSpecializationChange((e.target as HTMLInputElement).value)} 
							placeholder="Например: дизайн, верстка, разработка"
						/>
					</label>
					<div className="col-span" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-sm)' }}>
						<label>
							<span>Ставка</span>
							<TextInput
								mask="contractor-rate"
								value={rate}
								onChange={(e) => {
									onRateChange((e.target as HTMLInputElement).value);
									if (rateError) setRateError(null);
								}}
								onBlur={handleRateBlur}
								error={!!rateError}
								placeholder="Например: 2 300"
							/>
							{rateError && <span style={{ color: 'var(--red)', fontSize: 'var(--font-size-sm)', marginTop: 'var(--space-xs)' }}>{rateError}</span>}
						</label>
						<label>
							...
							<TextInput 
								...
								placeholder="1-5"
							/>
							{ratingError && <span style={{ color: 'var(--red)', fontSize: 'var(--font-size-sm)', marginTop: 'var(--space-xs)' }}>{ratingError}</span>}
						</label>
					</div>
					<label className="col-span">
						<span>Контакты</span>
					...
					<label className="col-span">
						...
						<NotesField 
							...
							placeholder="Заметки о подрядчике..."
							rows={3}
						/>
					</label>
					<label className="col-span">
