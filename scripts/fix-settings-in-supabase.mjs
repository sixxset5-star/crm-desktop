...
async function fixSettings() {
  try {
    ...
    if (allSettings && allSettings.length > 0) {
      ...
      // НО сначала сохраним настройки 'main' если она есть
      let mainSetting = null;
      const mainRecord = allSettings.find(s => s.key === 'main');
      if (mainRecord) {
        mainSetting = mainRecord;
        console.log('   📌 Сохраняем запись "main" для восстановления после удаления');
      }
      ...
      // Если была запись 'main' и мы ее удалили - восстановим после
      if (mainSetting && mainSetting.key === 'main') {
        // Восстанавливать не нужно - мы пересохраним правильные данные ниже
        console.log('   ℹ️  Запись "main" будет пересохранена в правильном формате');
      }
    } else {
