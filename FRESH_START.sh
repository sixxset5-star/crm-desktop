#!/bin/bash
# Создает новую чистую ветку без истории (проще чем filter-branch)
# ВНИМАНИЕ: Это создаст новую историю!

set -e

echo "🔄 Создаем новую чистую ветку без истории больших файлов..."

# Создаем новую orphan ветку (без родителей)
git checkout --orphan fresh-main

# Добавляем все файлы (кроме тех что в .gitignore)
git add .

# Делаем первый коммит
git commit -m "Initial commit - clean history without release files"

# Переименовываем старую ветку (на случай если нужно вернуться)
git branch -m main old-main-with-history 2>/dev/null || true

# Переименовываем новую ветку в main
git branch -m fresh-main main

echo ""
echo "✅ Новая чистая ветка создана!"
echo ""
echo "📤 Теперь нужно force push:"
echo "   git push -u origin main --force"
echo ""
echo "⚠️  Это перезапишет ветку main на GitHub!"
echo "   Старая ветка сохранена локально как 'old-main-with-history'"

